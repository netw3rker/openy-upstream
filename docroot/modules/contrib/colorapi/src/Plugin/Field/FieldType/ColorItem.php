...
class ColorItem extends FieldItemBase implements ColorItemInterface {
  ...
  public static function propertyDefinitions(FieldStorageDefinitionInterface $field_definition) {
    ...
    // Property contains color hexadecimal representation of color without #.
    $properties['value'] = $string_definition_info['definition_class']::create($string_data_type)
      ->setLabel('Value')
      ->setDescription('The hexadecimal color representation without #')
      ->setComputed(TRUE);
  ...
  public function setValue($values, $notify = TRUE) {
    ...
    $this->setHexadecimalValue($color_string, $notify);
   ...
   * {@inheritdoc}
   */
  public function setHexadecimalValue($color, $notify = TRUE) {
    $this->get('value')->setValue(substr($color, 1), $notify);
  }
